import { createStatelite } from "../src";
import { LoadPersistedStateMiddleware } from "../src/middlewares/LoadPersistedStateMiddleware";
import { PersistMiddleware } from "../src/middlewares/PersistMiddleware";
...
describe("createStatelite with persistence", () => {
	...
	test("should persist state to localStorage", () => {
		const store = createStatelite(
			{ theme: "light" },
			{ persistKey: "app_theme" },
			[LoadPersistedStateMiddleware, PersistMiddleware]
		);
